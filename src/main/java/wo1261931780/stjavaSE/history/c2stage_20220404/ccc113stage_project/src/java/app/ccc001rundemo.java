...
public class ccc001rundemo {
	public static final List<ccc002user> user_list = new ArrayList<>();
	...
	public static void add_movie(List<ccc001movie> x) {
		...
		x.add(x1);
		log.info("电影名称" + "\t" + "主演" + "\t" + "时间" + "\t" + "票价" + "\t" + "分数");
		log.info(x1.getMovie_name() + "\t" + x1.getMovie_actor() + "\t" + x1.getMovie_time() + "\t" + x1.getMovie_price() + "\t" + x1.getMovie_score());
		log.info("添加完毕");
	}
		...
		while (true) {
			...
			user_list.forEach(s -> {
				if (s.getAccount().equals(acc) && s.getPassword().equals(paw)) {
					...
					log.info("登录成功，欢迎" + s.getName() + title + "，正在进入操作界面……");
					login_user = s;
				...
				if (m1.getMovie_name().contains(change_title)) {// ee的标准，别这么low
					switch (command) {
						...
						case 4:
							...
							log.info("剩余电影票：" + m1.getMovie_remain_tickets());
							// ------------------
		...
		for (Map.Entry<ccc004business, List<ccc001movie>> demo : movie_list.entrySet()) {
			...
			for (ccc001movie result : demo1) {
				if (command == 1) {
					...
					log.info(result.getMovie_name() + "\t" + result.getMovie_actor() + "\t" + result.getMovie_time() + "\t" + result.getMovie_price() + "\t" + result.getMovie_score());
	...
	private static void del_movie(List<ccc001movie> demo, int command) {
		...
		for (ccc001movie m : demo) {
			...
			if (m.getMovie_name().contains(change_title)) {// ee的标准，别这么low
				...
				if (command == 7) {
					log.info("确定要下架电影" + m.getMovie_name() + "?Y/N");
					...
					log.info("修改成功！");
					log.info("电影名称" + "\t" + "主演" + "\t" + "时间" + "\t" + "票价" + "\t" + "分数");
					log.info(m.getMovie_name() + "\t" + m.getMovie_actor() + "\t" + m.getMovie_time() + "\t" + m.getMovie_price() + "\t" + m.getMovie_score());
					return;
				}
			}
		}
	...
	private static void show_login_acc_info(ccc002user x) {
		log.info("当前用户的名称：" + x.getName());
		log.info("当前用户的性别：" + x.getSex());
		log.info("当前用户的电话：" + x.getTel());
		log.info("当前用户的余额：" + x.getBalance());

		if (x instanceof ccc004business) {
			log.info("当前用户的店铺名称:" + ((ccc004business) x).getShop_name());
			log.info("当前用户的店铺地址:" + ((ccc004business) x).getAddress());
