...
//    }
    ...
    public void dos(){
        int jb;//这个变量在方法中，就是局部变量
        System.out.println("do sth.");
    }

    int cy;//这个变量在方法外，但是又在class类中，所以是成员变量。
    //成员变量在堆内存中。
    //有默认值
    public void xc(int nn){//这个nn就是形式参数，nn所在的括号，就是方法声明的位置。
        //调用方法的时候xc（10），这个10就是实际参数。
        System.out.println("nothing");
        int mm;//我们认为，mm和nn都是局部变量，因为都在方法内部。
    }
    //局部变量，在栈内存中。
